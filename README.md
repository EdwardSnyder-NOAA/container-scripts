...
4. Copy out the convert-modules.py file
   ...
   ```
   Notes: top-dir is the first dir in your $PWD. And you may have to module load singularity or apptainer first before running this command.
...
```
If things look in order, then run the following to build the externalized spack-stack with the host compilers:
```
...
To switch compilers, ensure that the new compilers are pre-loaded either on the host machine or by creating the Intel sandbox. Once they are loaded, run the following commands:
```
cd /path/to/modulefiles
./update_ss_container_compilers.sh -o /path/to/modulefiles/spack-stack-1.9.2 [-s <path to Intel sandbox>]
Note: drop the [-s] argument to load the host compilers
```
