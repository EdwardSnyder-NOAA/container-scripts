...
if [[ -z $(env | grep APPTAINER) ]]; then 
   ...
   sed -i 's/CONTAINERENV_/APPTAINERENV_/g' $PWD/run_modularized_executable.sh
   #replace the paths in the script
   sed -i "s|IMAGE|$APPTAINER_CONTAINER|g" $PWD/run_modularized_executable.sh
   nbinds=`echo $APPTAINER_BIND | awk -F "," '{print NF }'`
   sed -i "s/singularity exec/apptainer exec/g" $PWD/run_modularized_executable.sh
fi
bindstring=" "
